...
@section('content')
    <div>
        ...
        <div class="card-body mt" style="margin-top:-4vh">
            <div class="fieldset">
                <fieldset>
                    <div class="col-2 unverified-img">
            ...
            <div class="card-body" style="padding: 5px">
                <div class="row" style="text-align:right;">
                    <div class="col-12 d-flex justify-content-end gap-4">
                        ...
                        <div>
                            <form id="form_partner_verification" action="{{ route('partner.verify', Crypt::encrypt($partner->id)) }}" method="POST">
                                @csrf
                                <button type="button" class="btn btn-sm btn-warning text-nowrap px-5" id="btn_edit" style="max-width:17vw;border:2px solid cyan;"><i class="fa fa-edit"></i> <b>VERIFICATION</b></button>
                            </form>
                        </div>
                    </div>
                </div>
    ...
    <script>
        ...
        $(document).on('click', '.unverified-img img', function() {
            const src = $(this).attr('src');
            $('#img_modal').modal('show');
            $('#large-img').attr('src', src);
        });
        $(document).on('click', '.unverified-img', function() {
        ...
        $(document).on('click', '#btn_back', function() {
            ...
            $('#btn_back').text('Back to unverified list...');
            $('#btn_back').css('cursor', 'wait');
            location.href = "{{route('partner.unverified.index')}}";
        });
        $(document).on('click', '#btn_edit', function() {
            $('#btn_edit').attr('disabled', true);
            $('#btn_edit').text('Verifying...');
            $('#btn_edit').css('cursor', 'wait');
            $('#form_partner_verification').submit();
        });
    </script>
@endsection
