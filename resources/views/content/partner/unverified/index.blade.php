...
<div>
    <div class="d-flex justify-content-end py-1">
        <div>
            <form action="{{ route('partner.export', 'Partner Unverified') }}" method="get" style="height: 1rem;">
    ...
    <table id="unverified-partner-table" class="table table-hover mx-0 text-nowrap w-100">
        <thead>
            <tr>
                <th>No</th>
...
<script>
    ...
    const partnerTable = (partnerName="") => {
        $('#unverified-partner-table').DataTable({
            scrollY: 300,
            scrollX: true,
            processing: true,
            serverSide: true,
            ...
            bLengthChange: false,
            ajax: {
                url: "{{ route('partner.unverified.index') }}",
                data: {
    ...
    $(document).on('keyup', '#partner_name_filter', function() {
        $('#unverified-partner-table').DataTable().destroy();
        const partnerName = $(this).val();
        partnerTable(partnerName);
    });
    $(document).on('click', '.partner_names', function() {
        const base_url = "{{ url('/partner') }}";
        const id = $(this).attr('id');
        window.location.href = `${base_url}/unverified/${id}`;
    });
</script>
@endsection
